...
def main():
    def output_infection():
        with open("test/mega-virus_aligned.out.sam", "r") as file:
            count = 0
            virus_species = []
            for line in file:
        ...
        species_ratio = []
        stopper = 0
        reference = pd.read_csv('test/new_virus.species.txt', sep='\t', names=["id", "name"])
        reference = reference.set_index("id")
        for species in virus_species:
            count = 0
            with open("test/mega-virus_aligned.out.sam", "r") as file:
                # species_count.append(subprocess.check_output("grep -v @ " + "test/mega-virus_aligned.out.sam"
        ...
        output.to_csv(path_or_buf="test/output.csv")
